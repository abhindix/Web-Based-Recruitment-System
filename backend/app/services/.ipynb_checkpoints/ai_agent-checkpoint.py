...
If there are no results, say so clearly instead of guessing.
Never assume column names that are not guaranteed to exist.
"""
...
async def llm_chat(messages: list[dict]) -> dict:
    """
    Calls LM Studio / OpenAI-compatible chat endpoint directly.
    """
...
async def run_manager_agent(
    ...
    if name == "query_db":
        ...
        try:
            ...
            # ✅ FIX: handle bad AI SQL safely
            return (
                "There are currently no applicants for this job, "
                "or the requested details are not available yet.",
                {"sql": sql, "rows": []},
            )        
        ...
        msgs2 = msgs + [
            choice,
            {
                ...
                "name": "query_db",
                "content": json.dumps(
                    {"sql": sql, "rows": rows[:50]},
                    default=str,  # ✅ FIX: datetime-safe serialization
                ),
            },
        ]
